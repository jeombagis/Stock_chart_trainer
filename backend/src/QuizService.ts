...
import { IMarketDataProvider, INSTRUMENTS, Candle } from './MarketDataProvider';
import { saveQuiz, getQuizById, saveQuizAnswer, getRecentCutoffDates, getOverallStats, getCandlesFromDb, QuizRecord, StatsResponse } from './db';
...
export class QuizService {
  ...
  async createQuiz(config: QuizConfig = {}): Promise<QuizPublicData> {
    const instrument = config.instrument && INSTRUMENTS[config.instrument] ? config.instrument : 'sp500';
    const symbol = INSTRUMENTS[instrument].symbol;
    ...
    const recentCutoffs = new Set(await getRecentCutoffDates(symbol, 25));
  ...
  async answerQuiz(quizId: string, prediction: string): Promise<QuizAnswerResult> {
    ...
    if (quiz.status === 'ANSWERED') {
      const err: any = new Error('Quiz has already been answered');
      err.status = 400;
      throw err;
    }
    ...
    let revealedCandles: Candle[] = [];
    if (cutoffIndex !== -1 && endIndex !== -1) {
      revealedCandles = allCandles.slice(cutoffIndex + 1, endIndex + 1);
    } else {
      // Fallback
      revealedCandles = await getCandlesFromDb(quiz.symbol, quiz.cutoff_date, quiz.forecast_end_date);
