	...
	_internal_onNewRealtimeDataReceived() {
		...
		if (this._private__series._internal_options().lastPriceAnimation === 2) {
			...
			if (timeToAnimationEnd > 0) {
				if (timeToAnimationEnd < 650) this._private__endTime += 2600;
				return;
			}
...
var Series = class extends PriceDataSource {
	...
	_internal_applyOptions(options) {
		...
		merge(this._private__options, options);
		if (Object.prototype.hasOwnProperty.call(options, "autoscaleInfoProvider") && options.autoscaleInfoProvider === void 0) this._private__options.autoscaleInfoProvider = void 0;
	...
	*/
	...
	_internal_paneViewsForHoveredSourceOnTop() {
		...
		if (!isOverlay) normalPaneViews.push(this._private__baseHorizontalLineView);
			...
			if (sourceRange !== null) {
				switch (this._private__options.mode) {
					...
					case 2:
						...
						break;
					case 3: sourceRange = toIndexedTo100Range(sourceRange, firstValue._internal_value);
				}
...
var ChartModel = class {
	constructor(invalidateHandler, options, horzScaleBehavior) {
		...
		if (options.addDefaultPane) {
			...
			this._private__panes[0]._internal_setStretchFactor(2);
		}
...
function drawSourceViews(paneViewsGetter, drawRendererFn, source, pane) {
	drawPaneViews(paneViewsGetter(source, pane), drawRendererFn, pane);
}
...
var PaneWidget = class PaneWidget {
	...
	_private__drawSourceGroup(target, paneViewsGetter, drawFn, panePrimitives, sources, sourcePaneViewsOverride) {
		for (const panePrimitive of panePrimitives) this._private__drawSourceImpl(target, paneViewsGetter, drawFn, panePrimitive);
		if (sourcePaneViewsOverride === void 0) {
			for (const source of sources) this._private__drawSourceImpl(target, paneViewsGetter, drawFn, source);
			return;
		}
		for (const source of sources) this._private__drawSourceImpl(target, paneViewsGetter, drawFn, source, sourcePaneViewsOverride(source));
	}
	_private__hoveredSplitSource() {
		const state = ensureNotNull(this._private__state);
		const hoveredSource = state._internal_model()._internal_hoveredSource()?._internal_source;
		if (!state._internal_model()._internal_options().hoveredSeriesOnTop || hoveredSource === void 0) return null;
		for (const source of state._internal_orderedSources()) if (source === hoveredSource) return source;
		return null;
	}
	_private__hoveredSplitSourcePaneViews(source, pane) {
		...
		if (hoveredPaneViews === null || hoveredPaneViews._internal_topPaneViews.length === 0) return null;
	...
	_private__drawSourceImpl(target, paneViewsGetter, drawFn, source, paneViewsOverride) {
		...
		if (paneViewsOverride === void 0) drawSourceViews(paneViewsGetter, drawRendererFn, source, state);
		else drawPaneViews(paneViewsOverride, drawRendererFn, state);
	}
	...
	_private__applyTimeScaleInvalidation(invalidation, time) {
		...
		switch (invalidation._internal_type) {
			...
			case 4:
				...
				break;
			case 5: if (!invalidation._internal_value._internal_finished(time)) timeScale._internal_setRightOffset(invalidation._internal_value._internal_getPosition(time));
		}
	}
...
var ChartApi = class {
	...
	_private__sendUpdateToChart(update) {
		const model = this._private__chartWidget._internal_model();
		for (const series of update._internal_series.keys()) series._internal_invalidatePaneViewData();
...
var FillSeriesCompositeRenderer = class {
	...
	_internal_draw(target, isHovered, hitTestData) {
		...
		if (this._private__shouldDrawLine() && (!isHovered || !this._private__shouldDrawLineOnTop())) this._private__lineRenderer._internal_draw(target, isHovered, hitTestData);
	}
};
	...
	_private__fillPrecalculatedCache(pixelRatio) {
		...
		for (let i = this._private__data._internal_visibleRange.from + 1; i < this._private__data._internal_visibleRange.to; i++) {
			...
			if (current._internal_left - prev._internal_right !== spacing + 1) {
				if (prev._internal_roundedCenter > prev._internal_center) prev._internal_right = current._internal_left - spacing - 1;
		...
		if (spacing > 0 && minWidth < alignToMinimalWidthLimit) for (let i = this._private__data._internal_visibleRange.from; i < this._private__data._internal_visibleRange.to; i++) {
			const current = this._private__precalculatedCache[i - this._private__data._internal_visibleRange.from];
			if (current._internal_right - current._internal_left + 1 > minWidth) {
				if (current._internal_roundedCenter > current._internal_center) current._internal_right -= 1;
		...
		target.useMediaCoordinateSpace((scope) => {
			...
			switch (this._private__data.vertAlign) {
				...
				case "center":
					...
					break;
				case "bottom": vertOffset = Math.max(mediaSize.height - textHeight, 0);
			}
			for (const line of this._private__data.lines) {
				ctx.save();
...
function shapeSize(shape, originalSize) {
	return ceiledOdd(Math.max(originalSize, 12) * (shape === "circle" ? .8 : shape === "square" ? .7 : 1));
}
...
var SeriesMarkersPaneView = class {
	...
	_internal__makeValid() {
		...
		if (this._private__series.dataByIndex(0, 1) === null) return;
